...
CLI.prototype.createHub = function createHub(options) {
    return new Hub({
        ...
        mountpoint: options.get("mountpoint"),
    ...
    function prepareTests(client) {
        ...
        client.getAgents(function (err, agents) {
            ...
            if (agents.length === 0 && !browsers) {
                self.error("Waiting for agents to connect at " + externalUrl  + options.get("mountpoint"));
                if (externalUrl !== url) {
                    self.error("...also available locally at " + url  + options.get("mountpoint"));
                }
...
CLI.prototype.route = function (argv) {
    var parsedArgs = parseArgv(argv),
        ...
        usage = "usage: " + argv[1] +
                ...
                " [--wd-url <url>] [--self-url <url>] [--mountpoint <path>]" +
                " [--help] [--] [<HTML file>...]";
