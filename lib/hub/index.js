...
var Hub = module.exports = function Hub(options) {
    ...
    this.tower.installHandlers(this.server, {
        prefix: options.mountpoint ? (options.mountpoint + "/tower") : "/tower",
        log: this._onTowerLog.bind(this)
    });

    this.sessions = new EventYoshi();

    var _echoechoOptions = {
        all: true
    };
    if (this.option.mountpoint) {
        _echoechoOptions.paths = [this.option.mountpoint];
    }
    this.echoecho = new EchoEcho(_echoechoOptions);
    ...
    this.mountpoint = options.mountpoint ? options.mountpoint : "/";
